...
**Назначение:** Фоновый воркер, который периодически выбирает из таблицы `outbox_messages` неотправленные сообщения со статусом `pending` и публикует их в RabbitMQ. После успешной публикации статус сообщения меняется на `sent`. Реализует паттерн Transactional Outbox для гарантированной доставки сообщений.
...
[`src/di.py`](src/di.py) — контейнер внедрения зависимостей Dishka. Обеспечивает:
- Слабое связывание компонентов
- Упрощённое тестирование (замена зависимостей на моки)
...
Паттерн для гарантированной доставки сообщений в RabbitMQ:

1. При изменении состояния задачи в БД создаётся запись в таблице `outbox_messages`
2. Outbox Publisher Worker периодически выбирает неотправленные сообщения и публикует их в RabbitMQ
3. После успешной публикации статус сообщения меняется на `sent`
4. В случае ошибки сообщение остаётся в статусе `pending` для повторной попытки
...
Для пагинации списков используется библиотека `sqlakeyset`, реализующая курсорную пагинацию (keyset pagination). Преимущества перед offset-пагинацией:
- Стабильная производительность на больших объёмах данных
- Корректная работа при вставке/удалении записей во время пагинации
- Отсутствие проблемы «смещения» при параллельных изменениях